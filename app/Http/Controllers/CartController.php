...
class CartController extends Controller
{
    ...
    public function applyDiscount(Request $request)
    {
        ...
        if ($code->max_user > 0) {
            $couponUser = Order::where('coupon_code_id', $code->id)->count();
            if ($couponUser >= $code->max_uses) {
                return response()->json([
                    'status' => false,
        ...
        if ($code->max_uses_user > 0) {
            $couponUseByUser = Order::where(['coupon_code_id' => $code->id, 'user_id' => Auth::user()->id])->count();
            if ($couponUseByUser >= $code->max_uses_user) {
                return response()->json([
                    'status' => false,
                    'message' => 'You have already used this coupon',
                ]);
        ...
        if ($code->min_amount > 0) {
            if ($subTotal < $code->min_amount) {
                return response()->json([
                    'status' => false,
                    'message' => 'You min amount must be ' . $code->min_amount . ' MAD',
                ]);
            }
        }
